...
var region:     Region   = Region.BOARD    # where keyboard input currently goes
...
func move(dir: Vector2i, rack_size: int) -> void:
	if region == Region.BOARD:
		# Cross into the rack only if there is a tile to land on; an empty
		# rack would leave focus nowhere, so stay anchored on the board.
		if dir == Vector2i(0, 1) and board_pos.y >= BOARD_SIZE - 1 and rack_size > 0:
			region = Region.RACK
			rack_index = clampi(board_pos.x, 0, rack_size - 1)
		else:
